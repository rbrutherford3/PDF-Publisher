...
def resolve_api_key(api_key_override: str | None = None) -> str:
    if api_key_override is not None:
        candidate = (api_key_override or "").strip()
        if candidate:
            return candidate

    if resolve_sandbox_mode():
        return (os.environ.get("CLOUDCONVERT_SANDBOX_API_KEY") or os.environ.get("CLOUDCONVERT_API_KEY") or "").strip()
    return (os.environ.get("CLOUDCONVERT_LIVE_API_KEY") or os.environ.get("CLOUDCONVERT_API_KEY") or "").strip()
...
def convert_office_to_pdf(uploaded_file, api_key: str | None = None, filename: str | None = None, sandbox: bool | None = None) -> tuple[bytes, str]:
    ...
    try:
        cloudconvert.configure(api_key=resolved_api_key, sandbox=sandbox_mode)
