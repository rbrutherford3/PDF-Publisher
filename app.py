...
from cloudconvert_service import CloudConvertError, convert_office_to_pdf
...
def cloudconvert_test_page():
    return render_template(
        ...
        api_key_value=os.environ.get("CLOUDCONVERT_API_KEY", ""),
        error_message=None,
    )
...
def cloudconvert_test_convert():
    ...
    api_key = (request.form.get("api_key") or os.environ.get("CLOUDCONVERT_API_KEY") or "").strip()
    ...
    if recaptcha_passed:
        ...
        for f in filelist:
            ...
            if extension in {".doc", ".docx", ".odt"}:
                api_key = (os.environ.get("CLOUDCONVERT_API_KEY") or "").strip()
                try:
